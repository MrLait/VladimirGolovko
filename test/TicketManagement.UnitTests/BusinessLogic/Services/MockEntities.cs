...
namespace TicketManagement.UnitTests.BusinessLogic.Services
{
    ...
    {
        protected List<Venue> Venues { get; private set; }

        protected List<Layout> Layouts { get; private set; }

        protected List<Area> Areas { get; private set; }

        protected List<Seat> Seats { get; private set; }

        protected List<Event> Events { get; private set; }

        protected List<EventArea> EventAreas { get; private set; }

        protected List<EventSeat> EventSeats { get; private set; }

        internal Mock<IDbContext> Mock { get; private set; }
        ...
        public void InitialMock()
        {
            ...
            {
                new () { Id = 1, VenueId = 1, Description = "Layout for football games." },
            ...
            Events = new List<Event>
            {
                ...
                new Event
                {
                    Id = 2, Name = "Football match.", Description = "Netherlands - Belarus", LayoutId = 1, StartDateTime = new DateTime(3021, 04, 01), EndDateTime = new DateTime(3031, 04, 01),
                    ImageUrl = "Test",
                },
            };

            Mock = new Mock<IDbContext>();
            Mock.Setup(x => x.Venues.GetAllAsQueryable()).Returns(Venues.AsQueryable());
            Mock.Setup(x => x.Seats.GetAllAsQueryable()).Returns(Seats.AsQueryable());
            Mock.Setup(x => x.Layouts.GetAllAsQueryable()).Returns(Layouts.AsQueryable());
